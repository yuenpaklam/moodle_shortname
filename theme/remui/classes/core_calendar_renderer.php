...
class core_calendar_renderer extends \core_calendar_renderer
{
   ...
   /* public function add_pretend_calendar_block(block_contents $bc, $pos=BLOCK_POS_RIGHT) {
    ...
    public function course_filter_selector(moodle_url $returnurl, $label = null, $courseid = null)
    {
        ...
        $courseoptions[SITEID] = (object) [
                'labeltext' =>  get_string('fulllistofcourses'),
                'attributes' => array('data-content'=>'<div style="background:#eaeaea; color:#76838f">' . get_string('fulllistofcourses') .'</div>'),
              ];
        
        $bgpalette = ['#ede0f7','#c7dff9','#cae9e6','#e5edc6','#ffe4bf','#ffe0e0','#c0a2da','#8a9eda','#71b3ad','#94b577','#f19f6d','#ea8080'];
        $fontpalette = ['#9672b1','#4f7cc6','#1c8a7d','#439400','#ff8133','#ee5454','#fff','#fff','#fff','#fff','#fff','#fff'];
        
        $css = html_writer::start_tag('style');
        $css .= ".calevent-site {background-color: #666!important}\n";
        $css .= ".calevent-category {background-color: #888!important}\n";
        $css .= ".calevent-user {background-color: #AAA!important}\n";        
    
        $i=0;
        foreach ($courses as $course) {
            $coursecontext = context_course::instance($course->id);
            //$courseoptions[$course->id] = format_string($course->shortname, true, array('context' => $coursecontext));
            $labeltext = format_string($course->fullname, true, array('context' => $coursecontext));
            $courseoptions[$course->id] =(object) [
                'labeltext' => $labeltext,
                //'attributes' => array('data-content'=>'<div style="background:' . $bgpalette[$i % sizeof($bgpalette)] . '">' . $labeltext .'</div>'),
                'attributes' => array('data-content'=>'<div class="calevent-course courseid-' . $course->id . '">' . $labeltext .'</div>')
            ];
            $eleclass = ".calevent-course.courseid-" . $course->id;
            //$css .= $eleclass . "," .calevent-course.courseid-" . $course->id . " span {background-color:" . $bgpalette[$i % sizeof($bgpalette)] . "!important; color:" . $fontpalette[$i % sizeof($bgpalette)]  . "!important;}\n" ; 
            $css .= "$eleclass, $eleclass span, $eleclass i {background-color:" . $bgpalette[$i % sizeof($bgpalette)] . "!important; color:" . $fontpalette[$i % sizeof($fontpalette)] . "!important}\n";
            $i++;
        }
        ...
        $select = $css .html_writer::label($label, 'course', false, ['class' => 'm-r-1']);
        $select .= html_writer::select($courseoptions, 'course', $selected, false, ['class' => 'cal_courses_flt selectpicker'], false);
    ...
    public function subscription_details($courseid, $subscriptions, $importresults = '')
    {
        ...
        $strnever = get_string('never', 'calendar');
