...
function question_category_select_menu_jstree($contexts,  $notcatorsubcatof = -1){
    $content = \html_writer::start_tag('ul');
    foreach ($contexts as $context){
        $categories = get_categories_for_contexts($context->id, 'parent, sortorder, name ASC', false);   
        if (!empty($categories)){
            $content .= \html_writer::start_tag('li', array('id' => "0,$context->id"));
            $content .= $context->get_context_name(true, false);
            ...
            foreach($toplevelcategoryids as $topcatid){
                if ($topcatid!=$notcatorsubcatof){
                    $content .= \html_writer::start_tag('li', array('id' =>  $topcatid . ',' . $categories[$topcatid]->contextid));
                    $content .= $categories[$topcatid]->name;
                    if (!empty($countstring = $categories[$topcatid]->questioncount)){
                        $content .= " ($countstring)";
                    }
                    
                    $content .= build_sub_list($categories,$categories[$topcatid]->childids, $notcatorsubcatof);
...
function build_sub_list($categories, $childids, $notcatorsubcatof) {
    $content ='';
    if (!empty($childids)){
        $content .=  \html_writer::start_tag('ul');
        foreach($childids as $id){
            if ($id!=$notcatorsubcatof){
                $content .=  \html_writer::start_tag('li', array('id' =>  $id . ',' . $categories[$id]->contextid));
                $content .=  $categories[$id]->name;
                if (!empty($countstring = $categories[$id]->questioncount)){
                    $content .=  " ($countstring)";
                }
                $content .= build_sub_list($categories,$categories[$id]->childids, $notcatorsubcatof);
                $content .=  \html_writer::end_tag('li');
            }
        }
        $content .=  \html_writer::end_tag('ul');
 ...
 */
 //CORE HACK: changed SQL to return the top category with smallest sortorder
function question_get_default_category($contextid) {
    ...
    $category = $DB->get_records('question_categories',
            array('contextid' => $contextid, 'parent' => 0), 'sortorder', '*', 0, 1);
    if (!empty($category)) {
...
function get_categories_for_contexts($contexts, $sortorder = 'parent, sortorder, name ASC') {
    ...
    //CORE HACK: include subcategories question count in questioncount calculations
    $records =  $DB->get_records_sql("
            SELECT c.*, (SELECT count(1) FROM {question} q
                        WHERE c.id = q.category AND q.hidden='0' AND q.parent='0') AS prequestioncount
              FROM {question_categories} c
             WHERE c.contextid IN ($contexts)
          ORDER BY $sortorder");  //prequestioncount excludes question in subcategories
    
    foreach($records as $record) {  //reindex record by category id
        $record->questioncount = 0;
        $idRecords[$record->id] = $record;
    }
  
    foreach($idRecords as $idRecord){
        $ancestorcatid = $idRecord->id;
        $i=0; //loop escape. This should not be needed unless there are bugs in categories structure.
        while ($ancestorcatid != 0 && $i<20){                  
            $idRecords[$ancestorcatid]->questioncount += $idRecord->prequestioncount;
            $ancestorcatid = $idRecords[$ancestorcatid]->parent;
            $i++;
        }
    }
...
class question_edit_contexts {
    ...
    public static $caps = array(
        ...
        'import'=>array('moodle/question:import'),
        'export'=>array('moodle/question:export'));
